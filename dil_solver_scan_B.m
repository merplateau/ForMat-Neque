...
Tperp_e = 3*Te;        % 电子垂直温度 (K)
Tpara_e = 1*Te;          % 电子平行温度 (K)
Tperp_i = 3*Ti;        % 离子垂直温度 (K)
...
% 色散方程
function F = my_disp_eq(kpar, omega, omega_pe, omega_pi, c, A1e, A1i, V_e, V_i, n_e, n_i, omega_ce, omega_ci, w_par_e, w_par_i)
...
for i = 1:num_B
    ...
    try
        ...
        k0 = kpar_cold_sol+1+1i;
        
        
        % 求解kpar
        options = optimset('Display','iter','TolFun',1e-30,'TolX',1e-20);
        kpar_sol = fsolve(@(kpar) my_disp_eq(kpar, omega, omega_pe, omega_pi, c, A1e, A1i, V_e, V_i, n_e, n_i, omega_ce, omega_ci, w_para_e, w_para_i), k0, options);
